...
router.post('/quit_demand_decision', (req, res) =>
{
    ...
    for(let user_id in req.body)
    {
        ...
        if(req.body[user_id][0] === 'ignore')
            decision_saving_arr.push(db_func.ignore_quit_request(user_id));
        else if(req.body[user_id][0] === 'remove_access')
            decision_saving_arr.push(db_func.remove_access(req.body[user_id][1]));
        else continue;
    }
    ...
    .then((result) =>
    {
        ...
        res.redirect(301, '/view_quit_demands');
    })
    .catch((err) =>
    {
        console.error(err);
